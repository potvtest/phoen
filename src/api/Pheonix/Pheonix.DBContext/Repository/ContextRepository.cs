...
{
    public class ContextRepository<C> :
    IContextRepository
         where C : DbContext, new()
    {
        private C _context;

        public ContextRepository(C context)
        {
            _context = context;
        ...
        {
            IQueryable<T> query = this.FindBy(predicate);
            return query;
        }
        ...
        public virtual bool Update<T>(T entity, T oldModel) where T : class
        {
            ...
            foreach (string propertyName in entry.OriginalValues.PropertyNames)
            {
                ...
                if (!object.Equals(original, current) && current != null && propertyName != "ID" && propertyName != "SignInSignOutID" && propertyName != "PersonID" && propertyName != "ExpenseId" && propertyName != "Id")
                {
